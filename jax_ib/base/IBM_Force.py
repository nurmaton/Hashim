...
def IBM_force_GENERAL(field, Xi, particle, discrete_fn):
    ...
    # --- Calculate Physical Forces ON the Particle Boundary ---
    # 1. Penalty spring force on the fluid markers (X) from the mass markers (Y).
    penalty_force_x, penalty_force_y = calculate_penalty_force(xp, yp, Ym_x, Ym_y, Kp)
    
    # 2. Surface tension force on the fluid markers (X).
    ...
    tension_force_x, tension_force_y = jax.lax.cond(
        sigma > 0.0, compute_tension, no_tension, (xp, yp, sigma)
    )

    # Total force ON THE PARTICLE's boundary markers.
    force_on_particle_x = penalty_force_x + tension_force_x
    force_on_particle_y = penalty_force_y + tension_force_y
    
    # --- CORRECTED: Apply Newton's Third Law (Eq. 2) ---
    # The force exerted BY the particle ON THE FLUID is the negative of the force on the particle.
    force_on_fluid_x = -force_on_particle_x
    force_on_fluid_y = -force_on_particle_y
    # --- END CORRECTION ---
    
    force_to_spread = force_on_fluid_x if Xi == 0 else force_on_fluid_y

    # --- Spreading Logic (unchanged structure) ---
...
# --- REWRITTEN HIGH-LEVEL FUNCTIONS (unchanged from last step) ---
